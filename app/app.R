...
select_oao <- function(inputId, label, multiple = FALSE) {
  selectInput(
    ...
    choices = c(
      Vyberte = "",
      setNames(oao_names_tab$amcr_id, oao_names_tab$nazev)
    ),
    selectize = TRUE,
    multiple = multiple,
    width = "100%"
    ...
    column(
      ...
      fluidRow(
        column(
          6,
          HTML(
            "<b>Kliknutím do mapy</b> zvolte bod zájmu, 
      ...
      tabsetPanel(
          ...
          fluidRow(
            column(
              ...
              tags$div(
                ...
                HTML(
                  "<b> km</b> archeologický výzkum v posledních <b>5 letech</b>
        ...
        tabPanel(
          ...
          fluidRow(
            column(
              6,
              h4("Oprávnění ve vybrané oblasti"),
              tableOutput("tab_poly")
            ),
            column(
              6,
              h4("Oprávnění na celé území ČR"),
              tableOutput("tab_rep")
            )
          ),
...
details_page <- div(
  fluidRow(
    column(
      4,
      select_oao("oao", label = "Organizace:", multiple = FALSE),
      fluidRow(
        column(
          4,
          checkboxInput("poly", "Zobrazit působnost", value = TRUE)
        ),
        column(
          4,
        ...
        column(
          4,
          checkboxInput("addr", "Zobrazit sídlo", value = FALSE)
        )
      ),
      tags$hr(),
      uiOutput("detail"),
...
details_server <- function(input, output, session) {
  ...
  observe({
    if (!input$grid) {
      ...
      leafletProxy("map", data = oao_grid_flt()) %>%
        addPolygons(
          layerId = oao_grid_flt()$ctverec,
          color = ~ pal(scaled),
          stroke = FALSE,
          fillOpacity = 0.6
        ) %>%
        addControl(
          "<img src='legend.png' width=110 height=40>",
  ...
  output$link <- renderText({
    ...
    HTML(paste0(
      ...
      icon_link,
      " <b>",
      oao_names_tab[oao_names_tab$amcr_id == input$oao, ]$nazev,
      "</b></a><br>"
    ))
  })
...
# list ui
list_page <- div(
  select_oao("oao_multiple", label = "Filtrovat organizace:", multiple = TRUE),
  DT::dataTableOutput("table"),
  tags$p(
    style = "margin-top:6px;",
    "Celkem evidujeme ",
    textOutput("n_oao", inline = TRUE),
    " organizací."
  )
)
...
about_page <- div(
  fluidRow(
    ...
    column(
      4,
      includeMarkdown("text/about_center.md"),
      tagList(
        tags$h4("Cookies"),
...
ui <- fluidPage(
  ...
  tags$head(
    ...
    tags$meta(
      ...
      content = "MAXFZk91rhFItDNRVrQsdNapSLvo2r3W1Nf3zCImJbo"
    )
  ),
