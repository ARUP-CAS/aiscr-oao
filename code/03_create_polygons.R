...
separate2longer <- function(x, col, ncol) {
  x %>%
    separate(sym(col), into = str_c("c", seq(1, ncol, 1)), sep = ";") %>%
...
add_polygon <- function(x) {
  x %>%
    select(amcr_id, geometry) %>%
    group_by(amcr_id) %>%
    nest() %>%
    mutate(data = map(data, sf::st_as_sf), data = map(data, sf::st_union)) %>%
    unnest(data)
}
...
oao_uzemi_poly_simple <- sf::st_simplify(
  oao_uzemi_poly,
  dTolerance = 150,
...
if (
  file.exists(here::here("data/final", "oao_territory_poly_simple.geojson"))
) {
...
file.copy(
  here::here("data/final/oao_territory_poly_simple.geojson"),
  here::here("app/data/oao_scope.geojson"),
  overwrite = TRUE
)
