...
oao_api <- list.files(
  paste0(dir_data, "/input/"),
  pattern = "org",
  full.names = TRUE
) %>%
  as_tibble() %>%
  mutate(
    stamp = str_extract(value, "\\d{4}[-\\d{2}]*"),
    stamp = as.Date(stamp)
  ) %>%
  arrange(desc(stamp)) %>%
  first() %>%
...
oao_out <- oao %>%
  dplyr::mutate(
    ...
    uzemi = if_else(
      is_kraj & is_okres,
      paste0(kraj, " a okres ", okres, "."),
      uzemi
    ),
    uzemi = if_else(
      is_kraj & is_katastr,
      paste0(kraj, " a kat. úz. ", katastr, "."),
      uzemi
    ),
    uzemi = if_else(
      is_okres & is_katastr,
      paste0("Okres ", okres, " a kat. úz. ", katastr, "."),
      uzemi
    ),
    uzemi = if_else(
      is_kraj & is_okres & is_katastr,
      paste0(kraj, ", okres ", okres, " a kat. úz. ", katastr, "."),
      uzemi
    ),
    # proper dates
...
file.copy(
  paste0(dir_fin, "/oao_meta.geojson"),
